...
public class InfrastructureController {
	...
	public Page<Infrastructure> getInfrastructures(@RequestParam(required=false) Integer number, @RequestParam(required=false) Integer size) {
		if (number == null) number=0;
		if (size == null) size=IpplanConfig.DEFAULT_MAX_API_RESULT;		
		return repository.findAll(new PageRequest(number, size));
	}
